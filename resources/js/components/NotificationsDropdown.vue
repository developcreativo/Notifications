<!--
  - Copyright (c) 2024.  FaceIt
  - @author Kelly Salazar <developmentcreativo@gmail.com>
  -->
<template>
    <dropdown class="ml-auto h-9 flex items-center dropdown-right pl-10">
        <dropdown-trigger class="h-9 flex items-center">
        <span class="relative">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="ml-2 text-90 w-8">
                <path
            ...
            <span
                class="absolute -mt-4 -mr-1 text-xs bg-danger text-danger-light text-sm font-bold px-1 shadow-lg rounded-lg right-0 bottom-0"
                v-if="count > 0">
    ...
    export default {
        ...
        methods: {
            loadNotifications: function (callback) {
                axios
                    .get('/nova-vendor/notifications/unread')
                    .then(response => {
            ...
            markAllAsRead: function () {
                axios
                    .patch('/nova-vendor/notifications')
                    .then(() => {
                        this.notifications = []
                        this.count = 0
                    })
